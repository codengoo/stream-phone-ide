...
import { BrowserWindow, dialog, net as electronNet, ipcMain } from "electron"
...
ipcMain.handle(
  'stream:send-keyevent',
  async (_, { serial, keycode }: { serial: string; keycode: number }) => {
...
ipcMain.handle(
  'stream:set-rotation',
  async (_, { serial, rotation }: { serial: string; rotation: number }) => {
    const prefix = serial ? `adb -s ${serial}` : 'adb'
    try {
