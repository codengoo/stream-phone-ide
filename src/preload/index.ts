...
const streamAPI = {
  ...
  getSources: (): Promise<{ sources: Array<{ name: string; contentType: string; active: boolean }> }> =>
  ...
  getInfoFor: (serial: string, type = 'minicap') => ipcRenderer.invoke('stream:get-info-for', serial, type),

  takeSnapshot: (serial: string, type = 'minicap') => ipcRenderer.invoke('stream:take-snapshot-for', serial, type),

  getStreamUrl: (serial: string, type = 'minicap') => ipcRenderer.invoke('stream:get-stream-url', serial, type),
  ...
  getRandomPort: (): Promise<number> =>
    ipcRenderer.invoke('stream:get-random-port'),

  setRotation: (serial: string, rotation: number): Promise<boolean> =>
    ipcRenderer.invoke('stream:set-rotation', { serial, rotation }),

  switchSource: (source: string): Promise<boolean> => ipcRenderer.invoke('stream:switch-source', source),
}
